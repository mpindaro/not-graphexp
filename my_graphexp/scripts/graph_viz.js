	...
	function create_arrows(edge_in) {
		...
		_svg.selectAll('.arrow')
			...
			.attr('refX', function (d) {
				...
				return graphShapes.node_size(node)-6; // + graphShapes.node_stroke_width(node);
			})
			.attr('refY', 0)
...
//			var pinned_Nodes = _svg.selectAll("g").filter(".pinned");
			var pinned_Nodes = _svg.selectAll("g");
		...
		return {
			...
			set_fade_layers: set_fade_layers,
			depth: depth,
            fade: fade,
	...
	function refresh_data(d, center_f, with_active_node) {
		...
		d3.select(_nodes).each(function (d){
			centerTextNode(d)
		})
	...
	var graph_events = (function () {
		...
		function dragged(d) {
			...
			if (f_connected_edges._groups[0].length == 0) {
				...
				f_connected_edges
					.style("stroke-width", function () { return parseInt(d3.select(this).attr("stroke-width")); })
		...
		function clicked(d) {
			let element = d3.selectAll("g.node").filter(function(v){return d.id === v.id}).node()
			let circle = $(element.getElementsByTagName("circle")[1])
			if (circle.data("selected") == undefined) {
				selectedNodeElements.push(element)
				selectNode(d, element)
			}else if (!circle.data("selected")){
				selectedNodeElements.push(element)
				selectNode(d, element)
			}else{
				selectedNodeElements=selectedNodeElements.filter(v => v!=element)
				deselectNode(d,element)
			}
