...
var directed_graph = getUrlParameter('directed');
if (directed_graph == null) {
    directed_graph = "y"
} else {
    directed_graph = "n"
}
...
var graphShapes = (function () {
    ...
    function node_size(d) {
        ...
        let label = d.label
        let ruleLabel = rules[label]
        if (ruleLabel != undefined) {
            let sizeLabel = ruleLabel["dimension"]
    ...
    function node_stroke_width(d) {
        ...
        if ('stroke_width' in d.properties) {
            return d.properties.stroke_width.summary;
        } else {
    ...
    function node_color(d) {
        ...
        let ruleLabel = rules[label]
        if (ruleLabel != undefined) {
            let colorLabel = ruleLabel["color"]
    ...
    function node_text(d) {
        ...
        if ('node_text' in d) {
            return d.node_text;
        } else if ('node_subtext' in d.properties) {
    ...
    function decorate_node(node, with_active_node) {
        ...
        node_deco.append("text")
            .classed("text_details", true)
            ...
            .attr("fill", function (d) {
                //colore testo
                var tmp_col = node_color(d);
                tmp_col = ("0x" + tmp_col.slice(1));
                var rrr = tmp_col >> 16;
                var ggg = tmp_col >> 8 & 255;
                var bbb = tmp_col & 255;
                var hsp = Math.sqrt(0.299 * (rrr * rrr) + 0.587 * (ggg * ggg) + 0.114 * (bbb * bbb));
                if (hsp > 127.5) {
                    return '#000000';
                } else {
                    return '#FFFFFF';
            ...
            .style("font-family", function (d) {
                    ...
                    if (rules[label]!=undefined) {
                        let ruleLabel = rules[label]["icon"]
                        return ruleLabel != undefined ? "Material Icons" : "sans-serif"
                    }else{
                        return "sans-serif"
                    }
                })
            .style("font-size", function (d){
                ...
                if (rules[label]!=undefined) {
                    let ruleLabel = rules[label]["icon"]
                    return ruleLabel != undefined ? "25px" : "11px"
                }else{
                    return "11px"
                }
            })
        ...
        var node_pin = node_deco.append("circle").classed("Pin", true)
            .attr("r", function (d) {
                return node_size(d) / 2;
            })
            .attr("transform", function (d) {
                return "translate(" + (node_size(d) * 3 / 4) + "," + (-node_size(d) * 3 / 4) + ")";
            })
            .attr("fill", node_color)
            .moveToBack()
    ...
    function attach_node_actions(node) {
        ...
        node.on("click", graph_viz.graph_events.clicked)
            ...
            .on("mouseout", function () {
                ...
                //if (!show_checked)   NON voglio che la label del nodo scompaia se passo sul nodo e poi esco
                //	d3.select(this).selectAll(".text_details").style("visibility", "hidden");
            })
            .on("contextmenu", (d, i, p) => {
                nodeContextMenu(node, d3.event, d, i, p, this)
            });
        //centerTextNode(d3.select(node))

    }

    function decorate_link(edges, edgepaths, edgelabels) {

        var edges_deco = edges.append("path").attr("class", "edge").classed("active_edge", true).style("stroke", function (d) {
            return edge_color(d)
        })
            /*var edges_deco = edges.append("path").style("stroke", "#999").style("fill","none")*/
        ...
        if (directed_graph == "y") {
            ...
            edges_deco.attr('stroke-width', edge_stroke_width);
        }
    ...
    function attach_edge_actions(edge) {
        edge.on("mouseover", function () {
            ...
            .on("contextmenu", (d, i, p) => {
                edgeContextMenu(edge, d3.event, d, i, p, this)
            });
    ...
    function findMarkerOfEdge(id) {
        return Array.from(d3.selectAll(".arrow")._groups[0]).filter(v => v.__data__.id == (id))[0]
    }

    function changeEdgeColor(color, edge) {

        edge.style.stroke = color
        let marker = findMarkerOfEdge(edge.__data__.id)
        console.log(marker)
        marker.childNodes[0].style.fill = color
    }

    function changeEdgeDimension(dimension, edge) {
        edge.setAttribute("stroke-width", dimension)

    }

    function changeLabelTextInIcon(label, icon) {
        d3.selectAll(".text_details")
            .filter(function (d) {
                let ruleLabel = rules[label]
                if (ruleLabel != undefined) {
                    let icon = rules[label]["icon"]
                    if (icon != undefined) {
                        return d3.select(this).text().toLowerCase() == icon

                    } else {
                        return d3.select(this).text().toLowerCase() == label.toLowerCase()
                    }
                } else {
                    return d3.select(this).text().toLowerCase() == label.toLowerCase()
                }
    ...
    function getIconFontSize(d) {
        let obj = $(d3.select(d))
        return (obj.css("font-size") + 10) + "px"
    }

    function viewNodeColor(d) {
        return d3.select(d.parentNode).select(".base_circle").attr("fill")
    }

    function updateTextColors() {
        d3.selectAll("g.node").select("text").attr("fill", function (d, i) {
            //colore testo
            let tmp_col = viewNodeColor(this);
            if (tmp_col.length > 7) {
                let a = tmp_col.split("(")[1].split(")")[0];
                a = a.split(",");
    ...
    return {
        changeEdgeColor: changeEdgeColor,
        changeEdgeDimension: changeEdgeDimension,
        ...
        changeLabelTextInIcon: changeLabelTextInIcon,
