	...
	function addVertex()  {
		...
		if(nameLen != valueLen){
			...
			var gremlin_query = traversal_source+".addV('"+vertexLabel+"')"
	...
	//	vertexPropertyValue_1 = vertexPropertyValue_1.replace(/\s/g,'');
		...
		if(nameLen != valueLen){
			...
			var gremlin_query = traversal_source+".V('"+vertexId+"')"
			for(count =0; count<nameLen; count++){
				gremlin_query=gremlin_query+".property('"+propertyName[count]+"' , '" + propertyValue[count]+ "')"
		}
		console.log(gremlin_query)
	...
	function addEditEdge()  {
		...
		if(nameLen != valueLen){
			...
			var gremlin_query = traversal_source+".V('"+sourceVertexId+"').addE('"+edgeLabel+"').to(V('" +targetVertexId+"'))"
			for(count =0; count<nameLen; count++){
				gremlin_query=gremlin_query+".property('"+propertyName[count]+"' , '" + propertyValue[count]+ "')"
			}
