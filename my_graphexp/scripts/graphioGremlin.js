...
var graphioGremlin = (function(){
	...
	function getLabels(successHandler){
		let query = traversal_source + ".V().label().dedup()"
		send_to_server(query,null, null, null,successHandler )
...
//                        }
	...
	function get_graph_info(){
		...
		//TODO mettere un loading effettivo
	...
	function search_raw_query(query){
		send_to_server(query,'search',null, "");
	}
	...
	function search_query2(q) {
		...
		// Preprocess query
		let gremlin_query = $('#'+q).val();
//        let gremlin_query = "edges=bandi_traversal.V().hasLabel('persona_giuridica').has('piva','22222222222').outE().as('e').select('e').valueMap().select('anno','esito'); nodes=bandi_traversal.V().hasLabel('persona_giuridica').has('piva','22222222222').outE().inV().valueMap().select('id','den_lotto');[edges.toList(),nodes.toList()]"
        let communication_method = $('#communication_method').val();
		$('#messageArea').html('<h3>(loading)</h3>');
		// To display the queries in the message area:
		//var message_nodes = "<p>Node query: '"+gremlin_query_nodes+"'</p>";
		...
		//var message = message_nodes + message_edges;
		var message = "";
		send_to_server(gremlin_query,'table',null,message);
    }
	...
	function search_query() {
        ...
        let edge_filter = $('#edge_filter').val();
        let communication_method = $('#communication_method').val();
		...
		if (edge_filter !== "") {
			gremlin_query_edges = "edges = " + traversal_source + ".V(nodes).aggregate('node').outE().hasLabel('"+edge_filter+"').as('edge').inV().where(within('node')).select('edge').toList();";
		}
        ...
        //gremlin_query = "nodes=bandi_traversal.V().has('piva',within('11111111111','22222222222','33333333333')).toList(); edges=bandi_traversal.V(nodes).aggregate('node').outE().as('edge').toList();bandi=bandi_traversal.V(nodes).both('HA_PARTECIPATO').dedup().toList();[nodes+bandi,edges]"
		// while busy, show we're doing something in the messageArea.
		$('#messageArea').html('<h3>(loading)</h3>');
		// To display the queries in the message area:
		//var message_nodes = "<p>Node query: '"+gremlin_query_nodes+"'</p>";
		...
		if (SINGLE_COMMANDS_AND_NO_VARS) {
			...
			console.log("%cNode query:\n%s", "color: #ff7700", gremlin_query.toString())
			console.log("%cEdge query:\n%s", "color: #ff7700", gremlin_query.toString())
...
*/
	...
	function handle_server_answer(data,query_type,active_node,message){
		...
		if (query_type=='graphInfo'){
			...
			display_properties_bar(_node_properties,'nodes','Proprieta dei nodi:');
			display_properties_bar(_edge_properties,'edges','Proprieta dei link:');
			display_color_choice(_node_properties,'nodes','Colora i nodi per:');
		} else {
			//console.log(data);
			//console.log(graph)
            if (query_type == 'table') {
				var graph = arrange_datatable(data);
                infobox.show_table_data(graph);
//				$('#boxtabella').show();
            } else {
	...
	function arrange_datatable(data) {
		// Extract node and edges from the data returned for 'search' and 'click' request
		// Create the graph object
		var headers=[], dati=[], indice=0;
		if(data!=null) {
			for (var key in data){
				if(data[key]!=null) {
					if(key==0) {
					data[key].forEach(function (item) {
                        var valori = []
                        for (var kk in item) {
							if (headers.indexOf(kk) == -1) {headers.push(kk); }
							valori.push(item[kk]);
						}
						dati.push(valori);
					});
                     ...
                     if (typeof data[key] == "string") {
					  ...
					  indice = 0;
					  data[key].forEach(function (item) {
//                        var valori = dati[indice]
                        for (var kk in item) {
							if (headers.indexOf(kk) == -1) {headers.push(kk); }
							dati[indice].push(item[kk]);
//							valori.push(item[kk]);
						}
//						dati.push(valori);
                        indice = indice + 1;
	  ...
	  return {headers:headers, dati:dati, titolo: titolo};
	}
    ...
    function extract_infov3(data) {
        ...
        if (isGraphSON3_4) {
            ...
            if (data.type == 'vertex') {
                if (data.label == 'persona_giuridica') {
                    if ('ragione_sociale' in data_dic.properties) {
                        data.label = data_dic.properties['ragione_sociale'].summary;
                        data_dic.label = data_dic.properties['ragione_sociale'].summary;
                    } 
                }
                if (data.label == 'persona_fisica') {
                    if ('cognome' in data_dic.properties && 'nome' in data_dic.properties) {
                        data.label = data_dic.properties['cognome'].summary+" "+data_dic.properties['nome'].summary;
                        data_dic.label = data_dic.properties['cognome'].summary+" "+data_dic.properties['nome'].summary;
                    } 
                }
            }
        }
	if (data.type=="edge"){
        if (data.label == 'HA_PARTECIPATO') {
                    if ('esito' in data_dic.properties) {
                        data.label = data_dic.properties['esito'];
                        data_dic.label = data_dic.properties['esito'];
                    } 
                }
        if (data.label == 'COLLEGAMENTO') {
                    if ('documento' in data_dic.properties) {
                        data.label = data_dic.properties['documento'];
                        data_dic.label = data_dic.properties['documento'];
                    } 
                }
		data_dic.source = data.outV;
		data_dic.target = data.inV;
	...
	return {
		...
		getLabels: getLabels,
		search_raw_query: search_raw_query
	}
})();
